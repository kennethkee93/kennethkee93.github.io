...
export const InstallPwa: React.FC<{ className?: string }> = ({ className }) => {
  ...
  useEffect(() => {
    ...
    // Listen for the install prompt (only works if on the same domain as the app)
  ...
  const handleInstallClick = () => {
    if (deferredPrompt) {
      // Direct trigger for Android/Chrome/Edge (Same Origin)
      ...
      // If prompt isn't fired yet but it's not iOS, user might be on a different domain
      // OR the site isn't meeting PWA criteria yet.
      // We'll show a friendly message.
      alert("Installation is not available yet. Please ensure you are visiting the app's official URL or use your browser's menu to 'Add to Home Screen' manually.");
    }
  };
